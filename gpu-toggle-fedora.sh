...
NVIDIA_DEVICE_VIDEO_ID="2684"      # RTX 4090 Video (Check your own IDs!)
NVIDIA_DEVICE_AUDIO_ID="22ba"      # RTX 4090 Audio (Check your own IDs!)
...
DRACUT_CONF="/etc/dracut.conf.d/99-vfio.conf" # Fedora specific
...
# --- Setup / Status ---
run_setup(){
  log_action "Running Fedora/Nobara setup for VFIO (Dracut)..."
...
EOF
  ...
  # 5. Fedora Specific: Add modules to Dracut config
  log_action "Adding VFIO modules to $DRACUT_CONF..."
  cat > "$DRACUT_CONF" <<'EOF'
...
EOF
  ...
  log_action "Regenerating initramfs (dracut)..."
  if dracut -f; then
    log_action "Setup complete. Reboot to apply boot-time changes."
  else
    log_action "ERROR: dracut failed."
    return 1
  fi
  return 0
}
