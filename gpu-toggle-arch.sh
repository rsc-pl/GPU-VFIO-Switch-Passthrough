...
# Version: 2026-02-09 (CachyOS/Mkinitcpio Port)
...
# --- Setup / Status ---
run_setup(){
  log_action "Running CachyOS/Arch setup for VFIO (Mkinitcpio)..."
...
EOF
  ...
  # 5. Arch Specific: Add modules to mkinitcpio.conf if missing
  if [ -f "$MKINITCPIO_CONF" ]; then
      if ! grep -q "vfio_pci" "$MKINITCPIO_CONF"; then
           log_action "Adding VFIO modules to $MKINITCPIO_CONF..."
           sed -i 's/^MODULES=(/MODULES=(vfio_pci vfio vfio_iommu_type1 /' "$MKINITCPIO_CONF"
      fi
  else
      log_action "WARN: $MKINITCPIO_CONF not found. Is this standard Arch/CachyOS?"
  fi
  ...
  log_action "Regenerating initramfs (mkinitcpio)..."
  if mkinitcpio -P; then
    log_action "Setup complete. Reboot to apply boot-time changes."
  else
    log_action "ERROR: mkinitcpio failed."
    return 1
  fi
  return 0
}
